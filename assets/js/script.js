...
document.getElementById("logo").addEventListener("mouseover", function () {
	document.getElementById("logo").innerHTML = "DATA <span class='badge badge-success'>v1.2</span>";
})

document.getElementById("logo").addEventListener("mouseout", function () {
	document.getElementById("logo").innerHTML = "Do All TAsk <span class='badge badge-success'>v1.2</span>";
})
	...
	xml.onreadystatechange = function () {
		if (xml.readyState == 4 && xml.status == 200) {
			if (xml.responseText != "[]") {
				...
				document.getElementById("med").classList.remove("notification");
				document.querySelector("#notif").classList.remove("notification");
				$("#notif").html("Notification <i class='fa fa-sms'></i>");
			}
		}
	}
	xml.send()
...
{
		$("#sendEspece").on("click", function (e) {
			e.preventDefault();
			if($('#newEspece').val() == '')
			{
			...
			{
				xhr = getHttp();
				xhr.open("GET", "http://"+url+"espece/add/" + $("#newEspece").val())
